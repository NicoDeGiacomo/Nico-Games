...
function saveScoresTimer90(){
	...
	localStorage.setItem('topScoresTimer', JSON.stringify(mundo.topScoresTimer));
	localStorage.setItem('topScoresTries', JSON.stringify(mundo.topScoresTries));
	//remuevo ambos y guardo ambos o localStorage.removeItem(key); (remuevo uno solo)
}

function fillTable(){

	var types=["10T","20T","30T","30S","60S","90S"]

	for (var i = 0; i < types.length; i++) {

		switch(types[i]){
			case "10T":
				forToFill(mundo.topScoresTries.diez,types[i])
				break
			case "20T":
				forToFill(mundo.topScoresTries.veinte,types[i])
				break
			case "30T":
				forToFill(mundo.topScoresTries.treinta,types[i])
				break
			case "30S":
				forToFill(mundo.topScoresTimer.treinta,types[i])
				break
			case "60S":
				forToFill(mundo.topScoresTimer.sesenta,types[i])
				break
			case "90S":
				forToFill(mundo.topScoresTimer.noventa,types[i])
				break
		}
	};
	return
...
function forToFill(Things,type){

	for (var i = 0; i < Things.length; i++) {
		$("#"+type+"N"+i).text(Things[i].name)
		$("#"+type+"S"+i).text(Things[i].score)
	};
	return
}
